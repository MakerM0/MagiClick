...
//先通过计算的方式，得到实际图像数据的缓存，然后整块传输每一行数据
void  LCD_DrawMonoImgFast(u16 x,u16 y, u16 width,u16 height,const uint8_t pic[],u16 color,u16 bkcolor)
{
    ...
    for(uint16_t j=0;j<height;j++)
    {
        for(uint16_t i=0;i<width;i++)
        {
            if(((pic[w*j+i/8]>>(7-(i%8))) &0x01) == 0)
            {
                tmpbuf[i*2+1] =color&0x00ff;
                tmpbuf[i*2] =  (color>>8)& 0x00ff;
            }
            else
            {
                tmpbuf[i*2+1] =  bkcolor & 0xff;
                tmpbuf[i*2] =  (bkcolor>>8) &0x00ff;
            }
        }
        uint32_t k = 0;
        uint8_t * addr = (uint8_t *)tmpbuf;
        MySPIsendbuf(addr, width * 2);
    }
    LCD_CS_Set();
}
